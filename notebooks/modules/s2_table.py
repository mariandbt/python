...
def map_binnin(ev_x0, ev_y0, ev_z0, bin_width):

    x_nbins = int((ev_x0.max() - ev_x0.min())/bin_width)
    y_nbins = int((ev_y0.max() - ev_y0.min())/bin_width)

    xedges = np.linspace(ev_x0.min(), ev_x0.max(), x_nbins + 1)
    yedges = np.linspace(ev_y0.min(), ev_y0.max(), y_nbins + 1)
...
def create_s2_table(file_path, bin_width, s2_table_id):
    ...
    x_nbins, y_nbins, xedges, yedges = map_binnin(ev_x0, ev_y0, ev_z0, bin_width)
...
def create_response_maps(particles, sns_response, selected_sens, bin_width):
